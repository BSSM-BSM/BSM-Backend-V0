...
if ($_POST['post_no']==null||$_POST['boardType']==null) {
    ...
    for($i=0;$i<$result->num_rows;$i++){
        ...
        if($members_level[$comment['member_code']]>0){
            $member_level = $members_level[$comment['member_code']];
        }else{
            $member_level = '0';
        }
        if($anonymous_board){
        ...
        array_push($arr_comment, array('comment_idx' => $comment['comment_index'],'memberCode' => $comment['member_code'], 'memberNickname' => htmlspecialchars($comment['member_nickname'],ENT_QUOTES,'UTF-8'), 'memberLevel' => $member_level, 'comment' => htmlspecialchars($comment['comment'],ENT_QUOTES,'UTF-8'), 'commentDate' => $comment['comment_date']));
    }
