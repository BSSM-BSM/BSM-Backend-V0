...
if($_POST['boardType']==null){
    ...
    if ($_POST['post_no']==null) {
        ...
        if($post['post_deleted']==0){
            ...
            if($members_level[$post['member_code']]>0){
                $member_level = $members_level[$post['member_code']];
            }else{
                $member_level = '0';
            }
            ...
            $json = json_encode(array('status' => 1, 'post_title' => $post_title, 'post_content' => $post_content, 'member_code' => $member_code, 'member_nickname' => $member_nickname, 'memberLevel' => $member_level, 'post_comments' => $post_comments, 'post_hit' => $post_hit, 'post_like' => $post_like, 'like' => $like, 'post_date' => $post_date));
            echo $json;
        }else{
            statusCode(18);
        }
